...
class MODEL(tf.keras.Model):
    ...
    def call(self, inputs, is_training=None):
        ...
        n_sample = (
            self.args["n_train_sample"] if is_training else self.args["n_test_sample"]
        )

        # Generate noise samples
        noise = random.normal(
            shape=[n_sample, shape(r_mu)[0], self.args["z_dim"]], dtype=float32
        )
        ...
        return indiv_prob, self.nll_loss, self.marginal_loss, self.l2_loss, self.total_loss
